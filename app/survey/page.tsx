...
import { pulseChecks, PulseCheck, Question } from '@/lib/pulse-checks'

// ─── Age and country options ───────────────────────────────────────────────
...
const COUNTRIES = [
  ...
  'India', 'Singapore', 'South Africa', 'Nigeria', 'Kenya', 'Brazil', 'Mexico',
  'Other',
]
        ...
        <button
          ...
          className={`w-full text-left px-5 py-4 rounded-xl border-2 text-sm font-medium transition-all ${
            value === opt
              ...
              : 'border-gray-200 bg-white text-gray-700 hover:border-brand-muted hover:bg-gray-50'
          }`}
        >
          <span className="inline-flex items-center gap-3">
            <span className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
              value === opt ? 'border-brand-purple bg-brand-purple' : 'border-gray-300'
            }`}>
...
// Single choice — identical to scale but shown as cards
          ...
          >
            <span className="inline-flex items-center gap-3">
              <span className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center ${
                selected ? 'border-brand-purple bg-brand-purple' : 'border-gray-300'
              }`}>
                {selected && (
...
// Binary — large Yes / No buttons
function BinaryQuestion({ question, value, onChange }: {
  question: Question
  value: string
  onChange: (v: string) => void
}) {
        ...
        <button
          ...
          className={`flex-1 py-6 rounded-xl border-2 text-lg font-bold transition-all ${
            value === opt
              ...
              : 'border-gray-200 bg-white text-gray-700 hover:border-brand-purple hover:bg-brand-light'
          }`}
        >
          {opt}
        </button>
...
// Ranked choice — click options in order of preference
function RankedChoiceQuestion({ question, value, onChange }: {
  question: Question
  value: string[]   // ordered array — value[0] is 1st choice
  onChange: (v: string[]) => void
}) {
  const rank = (opt: string) => value.indexOf(opt) + 1  // 0 = not ranked

  const handleClick = (opt: string) => {
    if (rank(opt)) {
      // Already ranked — remove it and re-rank the remaining
      onChange(value.filter((x) => x !== opt))
    } else {
      // Not yet ranked — append to end
      onChange([...value, opt])
    }
  }
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-400 mb-1">
        Click each option in order of importance (1 = most important).
          ...
          >
            <span className="inline-flex items-center gap-3">
              <span className={`w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold ${
                r > 0 ? 'bg-brand-purple text-white' : 'bg-gray-100 text-gray-400'
              }`}>
                {r > 0 ? r : '–'}
              </span>
              {opt}
...
type Phase = 'loading' | 'closed' | 'survey' | 'demographics' | 'submitting' | 'done' | 'error'
...
export default function SurveyPage() {
  ...
  // ── Load the active pulse check ──
  useEffect(() => {
    // Prevent double-submission: if already done this session, redirect
    const done = sessionStorage.getItem('insight_pulse_done')
    if (done) {
      router.push('/thank-you?already=1')
      return
    }
    ...
    fetch('/api/active-pulse')
      .then((r) => r.json())
      .then((d) => {
        if (!d.activePulseCheck) {
          setPhase('closed')
        } else {
          const found = pulseChecks.find((p) => p.id === d.activePulseCheck)
          if (!found) {
            setPhase('closed')
          } else {
            setPulse(found)
  ...
  if (!pulse && phase === 'loading') {
    return <div className="flex items-center justify-center min-h-64 text-gray-400">Loading…</div>
  }
  ...
  if (phase === 'closed') {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 text-sm mb-6">Check back next week for the next Pulse Check.</p>
  ...
  function isCurrentAnswerValid(): boolean {
    if (phase === 'demographics') {
      return ageRange !== '' && country !== ''
    }
    const ans = getCurrentAnswer()
    if (!currentQ?.required) return true
    if (currentQ.type === 'multi_select') return Array.isArray(ans) && ans.length > 0
    if (currentQ.type === 'ranked_choice') {
      return Array.isArray(ans) && ans.length === currentQ.options.length
    }
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          ...
          <span>
            {phase === 'demographics'
              ? `${questions.length + 1} of ${totalSteps}`
              : `${step + 1} of ${totalSteps}`}
          </span>
        </div>
        <div className="w-full bg-gray-100 rounded-full h-1.5">
          <div
            className="bg-brand-pink h-1.5 rounded-full transition-all duration-500"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
        ...
        {phase === 'demographics' ? (
          // Demographics step
          <div>
            <p className="text-xs font-semibold text-brand-pink uppercase tracking-wide mb-1">
              Almost done
            </p>
            <h2 className="text-xl font-bold text-gray-800 mb-6">
              A couple of quick questions about you
            </h2>
            <p className="text-xs text-gray-400 mb-6">
              These help us understand our respondents better. Both fields are required.
            </p>
      ...
      <div className="flex items-center justify-between mt-6">
        <button
          onClick={handleBack}
          className="text-sm text-gray-400 hover:text-gray-600 font-medium px-4 py-2"
        >
          ← Back
        </button>
        ...
        {phase === 'demographics' ? (
          <button
            ...
            disabled={!isCurrentAnswerValid() || phase as string === 'submitting'}
          ...
          >
            {phase as string === 'submitting' ? 'Submitting…' : 'Submit my answers →'}
          </button>
        ) : (
          <button
            onClick={handleNext}
            ...
            className={`font-semibold text-white px-8 py-3 rounded-xl text-sm transition-all ${
              isCurrentAnswerValid()
                ? 'bg-brand-purple hover:bg-opacity-90 shadow-md'
                : 'bg-gray-200 text-gray-400 cursor-not-allowed'
            }`}
          >
            Next →
